...
class TestTextTokenProbabilityDatasetReader(AllenNlpTestCase):
    def test_text_to_instance_with_basic_tokenizer_and_indexer(self):
        ...
        self.assertEqual(
            [
                [t.text for t in masked_sentence]
        ...
        self.assertEqual(
            [
                [position.sequence_index for position in masks]
                for masks in instance["mask_positions"]
            ],
            [[0], [1], [2], [3], [4]],
        )
        ...
        # noinspection PyTypeChecker
        self.assertEqual(
            [t.text for t in instance["unmasked_sentence_tokens"]],
            ["This", "is", "a", "token", "."],
        )
